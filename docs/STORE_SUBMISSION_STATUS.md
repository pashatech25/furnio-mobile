...
- Signed production archive `output/store/Furnio-2026-09-22T14-21-31.764Z.xcarchive`, version 1.0.0 build 5, passed production configuration and codesign verification. Upload succeeded at 10:30 EDT; App Store Connect currently shows build 5 processing. Five framework dSYM warnings were non-blocking.
- Remaining Apple actions: wait for processing, select build 5, supply review screenshots/notes for the six approved native products, attach those products and the subscription group to version 1.0, update the reviewer explanation and submit. Studio1200 remains excluded.
