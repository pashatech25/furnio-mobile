...
## Fresh recording reset after Android authentication fixes — 13 September

- Owner explicitly requested another fresh signup recording reset. Deleted only the recreated `pashatheteq@gmail.com` account `c0f55dc9-a255-423d-b570-2af8ca2455b6`; revoked its sessions first, used the existing audited phone-release operation for +16479171234, and permanently removed its one project, two asset records, one completed job and seven exact owned media/temp objects. No payments or subscriptions existed. Auth returned 404 and account/profile/project/asset/job and object absence were verified; the number has no active/blocked claim. Main and reviewer accounts preserved. No downloaded backup was created.
- Recording handoff: when preparing another fresh Android signup recording after a fix, include the owner's requested reset of this exact test email and phone before handing it back. Resolve the current account ID each time, retain the scoped data/financial checks, and do not reset during ordinary testing or touch any other account.
- Supabase session/identity checks and Cloudflare object-ownership checks kept this reset limited to the authorized test account. No app code, website, Worker, schema or Apple submission changed.

## Android OAuth crash and deletion return — 13 September, 05:53 Toronto

- Android's separate crash buffer contained three real `Maximum update depth exceeded` crashes in Expo Router's nested navigation state, including 05:30:57. The ordinary recent log buffer initially did not show them. Symbolicated against the exact installed source map. Evidence: `output/android-auth-crash-before-2026-09-13.log`.
- Android now keeps its root Stack mounted and its key stable through identity/phone-loading transitions, covering it with the existing loading screen instead of repeatedly removing and rebuilding navigation. Protected screens still use the existing server-derived account/phone guards. iOS retains the previous key, loading branches and callback screens.
- Added Android-only callback routes using Expo's cached native linking URL. The previous late-mounted URL listener could miss the warm intent. The ordinary sign-in return now waits for account/trial hydration and explicitly selects the allowed dashboard, verification or recovery screen instead of replacing `/` mid-transition.
- Android browser-return coordination listens before opening and tolerates an AppState dismissal arriving just before the matching callback (bounded two-second cancellation grace). Exact callback destination/flow checks, PKCE and isolated deletion identity verification remain. Deletion's deduplicated handler remains until proof disposal so its late-mounted return screen can finish; it never replaces the main session. iOS delegates to the same unmodified two-argument browser call and keeps its previous handler cleanup.
- Full `pnpm check`: **1,155 tests / 78 files**, TypeScript and frozen API contracts passed; `git diff --check` passed. Regression cases include reordered callback parameters, resume-before-link, genuine cancellation, listener cleanup, foreign/stale flows, late deletion-route completion, unchanged iOS behavior and new-signup phone gating.
- Final production-configured local Android APK installed on `emulator-5554`: SHA256 `42a70a58c0f98ac8dc015ec852b2be8a5f818766f569d7afe48ecb640f646143`; build log `output/android-emulator-2026-09-13T09-48-21.214Z.log`. This is still a debug-key-signed Release for local testing, not a Play upload.
- Actual Google sign-out → account chooser → sign-in returned to Home without relaunch. PID `20200` remained unchanged. Home showed the existing account and **2 of 3** watermarked previews. Evidence: `output/android-google-signin-final.png`. New crash buffer since final installation was empty (`output/android-auth-final-crash-check.log`). No new signup, SMS or generation was performed by this check.
- Actual Google deletion reauthentication also passed on the **final installed APK**, returning to **Before you go**, showing **1 project, 2 stored assets, 0 unfinished jobs** (`output/android-deletion-final-verified.png`). PID `20200` remained unchanged and the crash buffer stayed empty. Stopped before any deletion request or confirmation; the account remains intact. The preceding same-fix APK passed this flow as well (`output/android-deletion-fix-result.png`).
- No Apple build/signing/upload, Apple callback source, website, Worker, database schema, provider configuration, prompt, credit or account mutation was made. Only normal test-account sign-in/out and temporary isolated reauthentication sessions were used. Supabase guidance preserved verified identity and session isolation rather than bypassing account checks.

## Android security callback correction — 13 September, 05:30 Toronto

- Fixed the disabled SMS/sign-in button after a successful Turnstile check. React Native WebView 13.16.1's modern Android bridge reports the message **origin**, whereas iOS/legacy Android reports the full document URL. The previous shared check rejected valid Android callbacks. Android now accepts only the exact owned origin or the existing challenge document; iOS still requires the document path. Navigation restrictions, random nonce, bounded payload, expiration and server verification remain unchanged.
- `pnpm check` passed **1,137 tests / 76 files**, TypeScript and frozen API contracts; `git diff --check` passed. Four added regression tests cover Android/iOS and rejected foreign or malformed sources.
- Rebuilt and installed the self-contained production-configured local Release APK on `emulator-5554` only. Log: `output/android-emulator-2026-09-13T09-18-03.034Z.log`; SHA256 `80ae491f9d4bf9bb669b81c77bc6087999a413ddb88f55930f0fd384bd85523d`. This remains a local debug-key-signed test APK, not a Play upload.
- Actual native verification: Turnstile displayed Success and the shared Challenge component enabled Sign in. Fresh accessibility capture confirms `enabled=true`, with both input fields empty after a cold launch: `output/android-security-bridge-fresh.xml` / `.png`. An earlier delayed capture recorded expiration and is not the enabled-button evidence. No synthetic login was submitted and no SMS was sent by this check. Owner still completes the real SMS flow.
- At the owner's explicit request, permanently removed the freshly recreated recording-test account after confirming zero payments, projects, media, jobs, sessions and pending deletion requests. Verified Auth 404, no profile, no Auth phone linkage and no active/blocked claim for the test number. Main and reviewer accounts were preserved. No schema, website, Worker, provider configuration or verification-rule changes.
- Emulator left signed out with the clean form for the owner's fresh Android recording. The Supabase safety checks limited this reset to the exact verified test account; the Turnstile guidance kept server-side validation and website isolation intact.

## Current — production emulator handoff, 13 September 2026

Prepared at the owner's request for hands-on Android testing before Google Play. The historical staging/demo notes below do not describe this installation.

- Regenerated **Android only** from the current Expo configuration using the guarded public production environment, then built the self-contained ARM64 Release APK. iOS, the Apple submission, website, Workers and database were not changed.
- Installed **1.0.0 / versionCode 2**, package `ai.furnio.app`, on `Pixel_9_Pro_XL_2` (`emulator-5554`, Android 16/API 36.1). The temporary emulator uses `-read-only -no-snapshot-save`; its changes are not saved after it closes. No device data was wiped.
- Embedded configuration verified as `production`. It uses the existing production customer APIs/authentication and shared accounts/credits, with native purchases disabled. This APK is debug-key-signed for local testing, **not a Google Play upload artifact**.
- Build: `output/android-emulator-2026-09-13T06-56-24.684Z.log`, successful in 2m29s. APK: `android/app/build/outputs/apk/release/app-release.apk`; SHA256 `d126648a9958e3439dbf0c59c157ec0c8df680214a8d1f99418cededc95d075d`. Package, embedded JavaScript/configuration and 16KB ZIP-alignment checks passed. Gradle deprecation warnings remain; ZIP alignment is not a physical-device or 16KB-runtime test.
- Current source checks: **1,133 tests / 76 files**, TypeScript and frozen customer API contracts passed.
- Cold launch succeeded. Inspected the actual Android screen: branded welcome, current three-edits promotion, Google/email sign-in and a successful security check. No `AndroidRuntime:E` or `ReactNativeJS:E` messages in the bounded startup log. Evidence: `output/android-production-2026-09-13-welcome.png`.
- Copied the seven previously prepared property/furniture test photos into `Pictures/Furnio-Test/` and verified all seven in Android's media index.

**Owner testing is next:** sign in, create a project with address lookup/map, pick a gallery/file image, run a service, compare and download the result, and verify balance refresh and sign-out/sign-in. No generation or production account mutation was performed during preparation. This startup check does **not** certify those end-to-end interactions; Google Play upload and closed testing remain separate steps. The review account must remain available to Apple and must not be used for deletion testing.
